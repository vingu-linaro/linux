...
static int sdp_ta_region_create(dma_addr_t addr, size_t size)
{
	...
	param[0].u.value.a = (uint32_t)((addr >> 32) & 0xFFFFFFFF);
	param[0].u.value.b = (uint32_t)(addr & 0xFFFFFFFF);
#else
	param[0].u.value.a = 0;
...
static int smaf_optee_destroy_context(void *ctx)
{
	...
	struct sdp_region *reg, *tmp;

	if (!client)
		return -EINVAL;

	list_for_each_entry_safe(reg, tmp, &client->regions_head, region_node)
		sdp_region_destroy(client, reg);
