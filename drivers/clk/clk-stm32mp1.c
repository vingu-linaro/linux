...
static const struct clock_config stm32mp1_clock_cfg[] = {
	/* Oscillator divider */
#ifdef HSI_FROM_SCMI
	FIXED_FACTOR(CK_HSI, "ck_hsi1", "clk-hsi", 0, 1, 1),
...
#undef WITH_POWER
#ifdef WITH_POWER
static int stm32_rcc_init_pwr(struct device_node *np, void __iomem *base);
#else
static int stm32_rcc_init_pwr(struct device_node *np, void __iomem *base)
{
	return 0;
...
static int stm32mp1_rcc_init(struct device_node *np)
{
	...
	ret = stm32_rcc_init_pwr(np, base);
	if (ret) {
		pr_err("TODO: cleanup RCC driver inits for later probe\n");
	}
...
static int stm32_rcc_init_pwr(struct device_node *np, void __iomem *base)
{
	...
	for (i = 0; i < ARRAY_SIZE(lp_table); i++)
		writel_relaxed(lp_table[i].val, base + lp_table[i].address);
