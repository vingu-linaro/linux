			    ...
			    bool tx)
{
	...
	// We don't expect Rx channel. Likely a std mailbox (interrupt+shm)
	pr_err(" ----------  SCMI OP-TEE channel setup ------------------\n");

	channel = devm_kzalloc(dev, sizeof(*channel), GFP_KERNEL);
	if (!channel)
		return -ENOMEM;

	shmem = of_parse_phandle(cdev->of_node, "shmem", (int)tx);
			      ...
			      struct scmi_xfer *xfer)
{
	...
	scmi_smt_tx_prepare(channel->shmem, xfer);

	ret = process_event(channel->channel_id);
	if (ret)
		return ret;

	scmi_rx_callback(cinfo, scmi_smt_read_msg_header(channel->shmem));
				 ...
				 struct scmi_xfer *xfer)
{
	...
	pr_err(" ----------  SCMI OP-TEE fetch response -----------------\n");
	scmi_smt_fetch_response(channel->shmem, xfer);
}
			    ...
			    struct scmi_xfer *xfer)
{
	...
	pr_err(" ----------  SCMI OP-TEE poll done ------------------\n");
	return scmi_smt_poll_done(channel->shmem, xfer);
}
